    ...
    if (SpeechRecognition) {
      ...
      recognition.onresult = (event: any) => {
        ...
        if (finalTranscript !== '') {
          ...
          baseTranscriptRef.current = base + (base && !base.endsWith(' ') ? ' ' : '') + finalTranscript;
        }
      ...
      recognition.onerror = (event: any) => {
        ...
        if (event.error !== 'not-allowed') {
          // Will try to restart on end if it wasn't a permission error
        } else {
      ...
      recognition.onend = () => {
        // If it stopped abruptly (not manually stopped), restart it
